...
$error=0;
	...
	*/
	...
	$res_db = mysqli_query($sql_db) or die("Error getting DB TABLES");
	while ($row_db = mysqli_fetch_row($res_db)) {
		//echo "Table: {$row[0]}\n";
	   	// now get fields within tables
	   	$this_table=$row_db[0];
		$sql_tables="SHOW COLUMNS FROM ".$row_db[0]."";
	  	$res_table = mysqli_query($sql_tables) or die("Error getting TABLE COLUMNS");
		if (mysqli_num_rows($res_table) > 0) {
			...
			while ($row_table= mysqli_fetch_assoc($res_table)) {
				...
				if(substr($this_field,-3,3)=="_en"){
					...
					if($res_check=mysqli_query($sql_check)){
						...
						mysqli_query($add) or die("Error ADDING AC_LANGUAGE");
	...
	if($error=="0"){
		header("Location:index.php?page=".ADMIN_PAGE."&id_added=".$new_lang."&msg=add_OK");
	} 
}
...
if(isset($_POST["delete_it"])){
	...
	$lang_file=AC_DIR_AC_LANG.$_POST["code"].".lang.php";
	//echo $lang_file."<br>";
	if(file_exists($lang_file)){
		unlink($lang_file) or die("unable to delete language file");
