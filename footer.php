    <footer>
        <div id="infos">
            <h1>Informations</h1>
              <p><a href="contact.php">Contact</a></p>
              <p><a href="boutique.php">Boutique</a></p>
              <p><a href="backoffice.php">Back-office</a></p>
        </div>
        <div id="rs">
          <ul>
            <li><a href="#" target="_blank"><i class="fa fa-facebook fa-3x" aria-hidden="true"></i></a></li>
            <li><a href="#" target="_blank"><i class="fa fa-pinterest fa-3x" aria-hidden="true"></i></a></li>
            <li><a href="#" target="_blank"><i class="fa fa-youtube fa-3x" aria-hidden="true"></i></a></li>
          </ul>
        </div>
        <div id="infos">
          <h1>Autres</h1>
          <p><a href="#">Mentions Légales</a></p>
          <p><a href="#">CGV</a></p>
          <p><a href="#">Plan du site</a></p>
        </div>

    </footer>
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
    <script src="assets/js/app.js"></script>
    <script src="assets/js/login.js"></script>
  </body>
</html>
